...
# RQ2: fuer EINE Session-Datei zwei Tabellen (plausibel gelesen / zu kurz),
# die je Reformulierungsstrategie zeigen, welche Interaktionstypen darauf folgen.
# Eingabe: cascade_full_1234_nostruct.tsv (default), RQ2_interactions_dwell.tsv

import csv, sys, re                          # re: fuer den Struktur-Filter
...
# Session-Datei (Argument oder Standard) und die Datei mit Interaktions-/Verweildauer-Info
...
# STRUCT: erkennt strukturierte/Bot-Anfragen (Jahr-Sweeps, Range-Filter, boolesche
# Operatoren). Solche Paare sind keine menschlichen Reformulierungen und werden
# ausgeschlossen -- identisch zum Filter in reformulation_examples.py, damit die
# Auswertungen auf DERSELBEN Basis stehen.
STRUCT = re.compile(r'year\s*:|yearpublished|(<=|>=|<|>)\s*\d|\bAND\s*\(|\bOR\s*\(',
                    re.IGNORECASE)

# Reihenfolge der Reformulierungsklassen fuer die Tabellenausgabe
...
# search_ids der Session-Datei sammeln (Zielmenge, auf die Interaktionen gemappt werden)
...
# Aus der Dwell-Datei je merge_key Bitmasken bauen: welche Interaktionstypen
# plausibel gelesen (pl) bzw. zu kurz (sh) auf diese Suche folgten
...
# Session-Datei paarweise durchgehen: jede Reformulierung klassifizieren und die
# darauf folgenden Interaktionstypen (plausibel/zu kurz) je Klasse zaehlen
def analyze(path, pl, sh):
    tot = Counter()
    hp = defaultdict(lambda: [0]*(len(TYPES)+1)); hs = defaultdict(lambda: [0]*(len(TYPES)+1))
    pairs = 0; skipped = 0                    # skipped: ausgeschlossene Struktur-Paare
    with open(path, newline="", encoding="utf-8") as f:
        r = csv.reader(strip_nul(f), delimiter="\t"); h = next(r)
        ...
        for row in r:
            ...
            # strukturierte/Bot-Paare ueberspringen (prev trotzdem fortschreiben)
            if STRUCT.search(prev) or STRUCT.search(row[i_q]):
                skipped += 1; prev = row[i_q]; continue
            # Paar klassifizieren und die Interaktions-Bitmasken je Typ verbuchen
            lbl = classify(prev, row[i_q]); tot[lbl] += 1; pairs += 1
            mp = pl.get(row[i_s], 0); ms = sh.get(row[i_s], 0); a = hp[lbl]; b = hs[lbl]
            for i in range(len(TYPES)):
                if mp >> i & 1: a[i] += 1
                if ms >> i & 1: b[i] += 1
            if mp: a[-1] += 1                  # letzte Spalte: irgendeine plausible Interaktion
            if ms: b[-1] += 1
            prev = row[i_q]
    return tot, hp, hs, pairs, sidcol, skipped

# Eine Ergebnistabelle ausgeben (je Strategie: Paare + Anteil je Interaktionstyp in %)
...
# Ablauf: Zielmenge der search_ids -> Interaktions-Masken -> Analyse -> zwei Tabellen
