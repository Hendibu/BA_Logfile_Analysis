...
# Eingabe: cascade_full_1234_nostruct.tsv (default), doc_titles.tsv | Ausgabe: known_item_beispiele.txt
...
# Pfade, Schwellen und die als "Verfeinerung" geltenden Reformulierungsklassen
...
# Hilfsfunktionen: NUL-Bytes entfernen, Text normalisieren, ersten passenden Spaltenindex finden
def strip_nul(fo):
...
# Titel-Index laden (normalisiert ins Set); fehlt die Datei, wird das Titel-Signal deaktiviert
titles=set(); nt=0
try:
...
# Kopfzeile der Session-Datei lesen und die Spalten session_id + query bestimmen
...
# Eine fertige Session klassifizieren: Titel-Treffer > Laenge/Verfeinerung entscheidet die Klasse
def finalize(qs):
    global title_sessions
    n=len(qs)
    title_hit=any(norm(q) in titles for q in qs if q.strip())
    if title_hit: title_sessions+=1
    refine=sum(1 for k in range(1,n) if classify(qs[k-1],qs[k]) in REFINE)   # Zahl der Verfeinerungs-Schritte
    if title_hit:
...
# Session-Datei durchlaufen, Queries je Block sammeln und jede fertige Session klassifizieren
with open(SRC, newline="", encoding="utf-8", errors="replace") as f:
...
# Verteilung der Klassen, Known-Item-Ausloeser und Exploratory-Unterteilung ausgeben
total=sum(labels.values())
print("KLASSIFIKATION JE SESSION:")
print("-"*52)
for lab in CLASSES:
...
# Bis zu 3 Beispiel-Sessions je Klasse in eine Textdatei schreiben
with open(OUTEX,"w",encoding="utf-8") as o:
    o.write("BEISPIEL-SESSIONS je Klasse\n"+"="*60+"\n")
    for lab in CLASSES:
        o.write(f"\n### {lab}\n")
