# DIS22-Session-ERSTELLUNG -- PRO uid|Tag (Eingabe dis22_uidday.tsv). Bei uid|Tag-Wechsel harte Grenze.
...
# #SORTED, OUT = "dis22_sorted.tsv", "dis22_sessions.tsv"
# SORTED, OUT = "dis22_sorted_noburst.tsv", "dis22_sessions.tsv"
# TIME_FILE   = OUT + ".time"
# MAX_ROWS    = None                          # None = alle
# TD_SEC, SERP_TH, COS_TH = 5*60, 0.01, 0.10       # td5 / Serp001 / Cos01
# BATCH, SAMPLE_FIT, MAX_FEAT = 20_000, 1_000_000, 100_000
...
# t0 = time.time()
# # --- Pass 1: Wort-TF-IDF auf Zufalls-Stichprobe lernen ---
# sample = []; n = 0
# with open(SORTED, newline="", encoding="utf-8") as f:
#     r = csv.reader(strip_nul(f), delimiter="\t"); h = next(r); iq = h.index("query")
#     for row in r:
...
# # --- Pass 2: streamend sessionisieren ---
# sid = 0; prev_vec = prev_serp = prev_ts = None; batch = []; seen = 0
# fo = open(OUT, "w", newline="", encoding="utf-8"); w = csv.writer(fo, delimiter="\t")
# w.writerow(["session_id", "ts", "query", "serp", "search_id", "uid"])

# def flush():
#     global sid, prev_vec, prev_serp, prev_ts, batch
#     if not batch: return
#     B = vec.transform([b[iq] for b in batch])
#     within = np.asarray(B[1:].multiply(B[:-1]).sum(axis=1)).ravel() if B.shape[0] > 1 else np.array([])
#     for k, b in enumerate(batch):
#         ts = int(b[its]); serp = parse_serp(b[isp])
#         if prev_vec is None:
#             sid = 1
#         else:
#             cos = float((B[0].multiply(prev_vec)).sum()) if k == 0 else float(within[k - 1])
...
#             if not same: sid += 1
#         w.writerow([f"session{sid}", ts, b[iq], b[isp], b[isid], b[iu]])
#         prev_vec = B[k]; prev_serp = serp; prev_ts = ts
#     batch = []
